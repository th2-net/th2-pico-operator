...
data class Queue(
    ...
    companion object {
        ...
        private const val EMPTY_QUEUE_NAME = ""
        private const val QUEUE_NAME_PREFIX = "link"

        private val QUEUE_NAME_REGEXP =
            "$QUEUE_NAME_PREFIX\\[$NAMESPACE_REGEXP:$BOX_NAME_REGEXP:$PIN_NAME_REGEXP]".toRegex()

        fun format(schemaName: String, boxName: String, pinName: String): String {
            return if (schemaName == "" && boxName == "" && pinName == "") {
                EMPTY_QUEUE_NAME
            } else {
                "$QUEUE_NAME_PREFIX[$schemaName:$boxName:$pinName]"
            }
        }

        fun fromString(str: String): Queue? {
            if (str.matches(QUEUE_NAME_REGEXP)) {
                try {
                    val enclosedStr = str.substring(QUEUE_NAME_PREFIX.length + 1, str.length - 1)
                    val tokens = enclosedStr.split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                    return Queue(tokens[0], tokens[1], tokens[2])
